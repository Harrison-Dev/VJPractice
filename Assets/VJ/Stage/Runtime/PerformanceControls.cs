...
public sealed partial class VJStage {
 public int LyricMode;public float Fps{get;private set;}public float P95{get;private set;}int parameter;RemoteDeck remote;
 ...
 void DrawPerformancePage(){
  ...
  if(Action(1120,351,145,"N 下一句重抽"))ApplyControl("motionReroll",0);
  if(Action(1273,351,145,"L 鎖定本句",MotionLocked))ApplyControl("motionLock",0);
  ...
  Caption(1120,482,MotionStatus,12,cyan,298);
  ...
  if(Action(1120,662,145,"開啟 iPad 控制"))Application.OpenURL(remote.Url);
  if(Action(1273,662,145,"複製連線網址")){GUIUtility.systemCopyBuffer=remote.Url;Message="已複製 iPad 控制網址。";}
