...
public sealed class RemoteDeck:MonoBehaviour {
 ...
 [Serializable] class State {public float energy,density,flow,echo,position,duration,bpm,fps,p95,low,mid,high;public int visual,lyric,motionMood;public bool playing,frozen,blackout,motionEnabled,motionLocked,motionPending;public string title,status,motionStatus;}
 ...
 void Update(){if(!stage)return;for(int i=0;i<32&&queue.TryDequeue(out var json);i++){try{var c=JsonUtility.FromJson<Command>(json);if(c!=null&&LyricDocument.Finite(c.value))stage.ApplyControl(c.action,c.value);}catch{}}if(Time.unscaledTime<nextPublish)return;nextPublish=Time.unscaledTime+.15f;var a=stage.Audio;stateJson=JsonUtility.ToJson(new State{energy=stage.Energy,density=stage.Density,flow=stage.Flow,echo=stage.Echo,position=stage.Position,duration=stage.Duration,bpm=stage.Bpm,fps=stage.Fps,p95=stage.P95,low=a.Bands.x,mid=a.Bands.y,high=a.Bands.z,visual=stage.TemplateIndex,lyric=stage.LyricMode,playing=stage.Playing,frozen=stage.Frozen,blackout=stage.Blackout,title=stage.Document.title,status=a.Status,motionEnabled=stage.KineticLyrics,motionMood=stage.MotionMoodIndex,motionLocked=stage.MotionLocked,motionPending=stage.MotionPending,motionStatus=stage.MotionStatus});}
